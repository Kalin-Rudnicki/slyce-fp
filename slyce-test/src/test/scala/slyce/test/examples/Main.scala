  ...
  val test: Executable = { (logger, _) =>
    for {
      ...
      marks = List(
        ...
        Marked("(1) 2 messages in same spot", Span(Span.Pos(16, 2, 6), Span.Pos(20, 2, 10)).some),
        Marked("(2) 2 messages in same spot", Span(Span.Pos(16, 2, 6), Span.Pos(20, 2, 10)).some),
        //

...
package object examples {
  ...
  final class GenerateConf(args: Seq[String]) extends Executable.Conf(args) {
    val debugOutput: ScallopOption[Boolean] = opt()
    val fileOutput: ScallopOption[Boolean] = opt()

    verify()
  }
  object GenerateConf extends Executable.ConfBuilder(new GenerateConf(_))
  ...
  def debugGenerate(
    ...
    Executable.fromConf(GenerateConf) { (logger, conf) =>
      for {
        _ <- logger(
          L(
